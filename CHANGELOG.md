...
### Fixed
- **Electron Build**: Forced Prisma to use `binary` engine type via `PRISMA_CLIENT_ENGINE_TYPE` environment variable to resolve module loading issues in packaged app.
...
- **Electron Build**: Updated server path configuration to support non-ASAR structure.



### Fixed
- **Electron Build**: Fixed startup crash by unpacking server dependencies from ASAR.
- **Electron Build**: Configured correct path for spawned server process in production.
- **Database**: Automatically configure DATABASE_URL for the internal server in the desktop app.



### Added
- **Factory Reset**: Added ability for administrators to reset all settings and data to factory defaults via the Settings page.
- **Setup Wizard**: Refactored setup and install pages to use client components and improve structure.

### Changed
- **Settings**: Added "Factory Reset" section to the General Settings tab.
- **Setup**: Improved the installation flow and UI.
